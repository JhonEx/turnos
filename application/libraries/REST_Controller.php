...
abstract class REST_Controller extends CI_Controller
{
	...
	/**
	 * Instnacia del entity manager de Doctrine
	 *
	 * 
	 */
	protected $em;
    ...
    protected function getTranslations($origin, $idOrigin, array $excluded){
        $return   = array();
        
        $object = $this->em->find("models\\".$origin, $idOrigin);
        
        if ($object){
            $data = $object->toArray();
            $this->loadRepository("Translations");
            
            $languages  = $this->config->config['languages'];
            $defaulLang = $this->config->config['language'];
            
            foreach($languages as $aLanguage){
                if ($aLanguage == $defaulLang){
                    continue;
                }
                
                foreach($data as $field => $value){
                    if (in_array($field, $excluded) == false){
                        $paremeters = array();
                        $paremeters["language"] = $aLanguage;
                        $paremeters["origin"]   = $origin;
                        $paremeters["idOrigin"] = $data["id"];
                        $paremeters["field"]    = $field;
                        $translation = $this->Translations->findOneBy($paremeters);
                        if ($translation){
                            $return[str_replace("-","_", $aLanguage)."_".$field] = $translation->getTranslation();
                        }
                    }
                }
            }
    ...
    protected function setTranslations($origin, $idOrigin, array $translations){
        $this->loadRepository("Translations");
        
        foreach ($translations as $lang => $data){
            foreach($data as $field => $tranlation){
                $translationObj = $this->Translations->findOneBy(array("origin"=>$origin, "idOrigin"=>$idOrigin, "field"=>$field,"language"=>$lang));
                $translationObj = ($translationObj) ? $translationObj : new models\Translations(); 
                $translationObj->setOrigin($origin);
                $translationObj->setIdOrigin($idOrigin);
                $translationObj->setField($field);
                $translationObj->setLanguage($lang);
                $translationObj->setTranslation($tranlation);
                $this->em->persist($translationObj);
            }
        }
