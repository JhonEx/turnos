<?php
$html = form_open('/sections/persistSectionPermission/', array("name"=>"form", "id"=>"form", "class"=>"form-horizontal"), array('id' => $id));
...
foreach ($sections as $aSection){
    if (count($aSection->getPermissions()) > 0){
        ...
        foreach ($aSection->getPermissions() as $aPermission){
            $selected = false;
            $disabled = "";
            foreach ($permissionsSection as $aPermissionSection) {
                $selected = ($aPermission->getId() == $aPermissionSection->getId());
                if ($selected){
                    $disabled = "disabled";
                    break;
                }
            ...
            $html .= form_checkbox(array("name" => "permission[]","value" => $aPermission->getId(),"checked" => $selected, "class" => "uniform_on", $disabled=>$disabled));
